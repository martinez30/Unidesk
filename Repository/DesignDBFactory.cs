...
{
    public abstract class DesignDBFactory<T> : IDesignTimeDbContextFactory<T> where T : DbContext
    {
        public T CreateDbContext(string[] args)
        {
            var basepath = Directory.GetCurrentDirectory() + string.Format("{0}..{0}Unidesk", Path.DirectorySeparatorChar);
